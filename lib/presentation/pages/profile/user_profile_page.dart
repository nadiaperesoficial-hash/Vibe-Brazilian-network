...
class _ProfilePageState extends State<UserProfilePage> {
    ...
    return ValueListenableBuilder(
      ...
      builder: (context, bool rebuildUserInfoValue, child) =>
        ...
        buildWhen: (previous, current) {
          if (previous != current && current is CubitUserLoaded) return true;
  ...
  Widget messageButtonForWeb() {
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Text(StringsManager.message.tr,
  ...
  Widget followButtonForWeb() {
    return BlocBuilder<FollowCubit, FollowState>(
      builder: (context, stateOfFollow) {
        bool isFriend = myPersonalInfo.followedPeople.contains(userInfo.value.userId) &&
            userInfo.value.followerPeople.contains(myPersonalId);
        bool isFollowing = myPersonalInfo.followedPeople.contains(userInfo.value.userId);
        bool isFollowLoading = stateOfFollow is CubitFollowThisUserLoading;

        String label = isFriend ? 'Amigos' : isFollowing ? 'Seguindo' : 'Adicionar amigo';

        Widget child = isFollowLoading
            ? const CupertinoActivityIndicator(color: ColorManager.black)
            : Text(label, style: getMediumStyle(color: isFriend || isFollowing ? ColorManager.black : ColorManager.white));
        ...
        return ValueListenableBuilder(
          ...
          builder: (_, UserPersonalInfo userInfoValue, _) => GestureDetector(
            ...
            child: Container(
              padding: EdgeInsets.symmetric(horizontal: 9, vertical: isFriend || isFollowing ? 4 : 6),
              decoration: BoxDecoration(
                color: isFriend || isFollowing ? ColorManager.transparent : ColorManager.blue,
                border: isFriend || isFollowing ? Border.all(color: ColorManager.lowOpacityGrey, width: 1) : null,
                borderRadius: BorderRadius.circular(3),
              ),
              child: child,
            ),
  ...
  Widget whichContainerOfText(FollowState stateOfFollow, UserPersonalInfo userInfoValue) {
    ...
    bool iFollowThem = myPersonalInfo.followedPeople.contains(userInfoValue.userId);
    bool theyFollowMe = userInfoValue.followerPeople.contains(myPersonalId);
    bool isFriend = iFollowThem && theyFollowMe;
    bool isFollower = !iFollowThem && theyFollowMe;
    ...
    return containerOfFollowText(text: label, isThatFollowers: isActive);
  }
  ...
  Container containerOfFollowText({required String text, required bool isThatFollowers}) {
    return Container(
      height: 35.0,
      decoration: BoxDecoration(
