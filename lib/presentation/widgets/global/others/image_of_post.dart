...
class _ImageOfPostState extends State<ImageOfPost>
  ...
  Future pushToProfilePage(Post postInfo) {
    if (widget.popupWebContainer) Navigator.of(context).maybePop();
  ...
  Widget buildPostForMobile({required double bodyHeight}) {
    return SizedBox(width: double.infinity, child: buildNormalPostDisplay(bodyHeight));
  }
  ...
  Widget buildPostForWeb({required double bodyHeight}) {
    return GestureDetector(
      onTap: () { showCommentBox = false; Navigator.of(context).maybePop(); },
      child: Scaffold(
                  ...
                  children: [
                    Flexible(
                      child: GestureDetector(
                        onTap: () => setState(() => playTheVideo = !playTheVideo),
                        child: Container(
                    ...
                    Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          Container(
                            decoration: const BoxDecoration(border: Border(bottom: BorderSide(color: ColorManager.black38, width: 0.08))),
                            child: Padding(
                              padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 15),
                              child: buildPublisherInfo(bodyHeight, postInfoValue, makeCircleAvatarBigger: true),
                            ),
                          ),
                          CommentsOfPost(
                            postInfo: widget.postInfo,
                          ...
                          Container(
                            decoration: const BoxDecoration(border: Border(top: BorderSide(color: ColorManager.black38, width: 0.08))),
                            child: Padding(
  ...
  Padding closeButton() {
    return Padding(
      ...
      child: GestureDetector(
        onTap: () { showCommentBox = false; Navigator.of(context).maybePop(); },
        child: const Align(alignment: Alignment.topRight, child: Icon(Icons.close_rounded, size: 26, color: ColorManager.white)),
      ),
    );
  }
  ...
  Widget loveButton(Post postInfo) {
    ...
    return GestureDetector(
      onTap: () async {
        setState(() {
          if (isLiked) {
            BlocProvider.of<PostLikesCubit>(context).removeTheLikeOnThisPost(postId: postInfo.postUid, userId: myPersonalId);
            postInfo.likes.remove(myPersonalId);
            if (widget.rebuildPreviousWidget != null) widget.rebuildPreviousWidget!();
            BlocProvider.of<NotificationCubit>(context).deleteNotification(notificationCheck: createNotificationCheck(postInfo));
          } else {
            ...
            if (widget.rebuildPreviousWidget != null) widget.rebuildPreviousWidget!();
            BlocProvider.of<NotificationCubit>(context).createNotification(newNotification: createNotification(postInfo));
          }
        });
      },
      child: !isLiked
  ...
  NotificationCheck createNotificationCheck(Post postInfo) {
    return NotificationCheck(senderId: myPersonalId, receiverId: postInfo.publisherId, postId: postInfo.postUid);
  }
  ...
  CustomNotification createNotification(Post postInfo) {
    ...
    return CustomNotification(
      ...
      postImageUrl: postInfo.postUrl.isNotEmpty ? postInfo.postUrl : postInfo.imagesUrls.isNotEmpty ? postInfo.imagesUrls[0] : '',
  ...
  bool _isUrl(String text) => text.startsWith('http://') || text.startsWith('https://');

  Widget imageOfPost(Post postInfo) {
    bool isLiked = postInfo.likes.contains(myPersonalId);
    final bool isTextOrLinkPost = postInfo.postUrl.isEmpty && postInfo.imagesUrls.isEmpty;
            ...
            ? Padding(
                ...
                child: AnyLinkPreview(
                  ...
                  errorWidget: Padding(
                    ...
                    child: Text(postInfo.caption, style: const TextStyle(color: Colors.blue, decoration: TextDecoration.underline)),
                  ),
      ...
      children: [
            ...
            child: Padding(
              ...
              child: postInfo.imagesUrls.length > 1
                  ? ImagesSlider(
                      ...
                      updateImageIndex: (index, _) => initPosition.value = index,
                    )
                  : postInfo.isThatImage
                      ? CustomNetworkImageDisplay(
                          blurHash: postInfo.blurHash,
        ...
        if (isHeartAnimation)
          LikePopupAnimation(
            ...
            child: const Icon(Icons.favorite, size: 100, color: ColorManager.white),
          ),
  ...
  Widget saveButton() {
    return ValueListenableBuilder(
      ...
      builder: (context, bool isSavedValue, child) => GestureDetector(
        ...
        child: iconsOfImagePost(IconsAssets.menuIcon),
      ),
    );
  }
  ...
  Widget commentButton(BuildContext context, Post postInfo) {
    return GestureDetector(
      onTap: () {
        if (isThatMobile) Go(context).push(page: CommentsPageForMobile(postInfo: widget.postInfo));
      },
                ...
                children: [
                  if (isMyPost) ...[
                    ListTile(
                      ...
                      title: Text(StringsManager.delete.tr, style: const TextStyle(color: Colors.red)),
                      onTap: () {
                        Navigator.pop(ctx);
                        PostCubit.get(context).deletePostInfo(postInfo: postInfo);
                        if (widget.removeThisPost != null) widget.removeThisPost!(widget.indexOfPost);
                      },
                    ),
                    ListTile(
                      leading: const Icon(Icons.edit),
                      title: Text(StringsManager.edit.tr),
                      onTap: () {
                        Navigator.pop(ctx);
                        Go(context).push(page: UpdatePostInfo(oldPostInfo: widget.postInfo));
                      },
                    ),
                  ] else ...[
                    ListTile(
                      leading: const Icon(Icons.person_add),
                      title: Text(StringsManager.follow.tr),
                      onTap: () {
                        Navigator.pop(ctx);
                        BlocProvider.of<FollowCubit>(context).followThisUser(followingUserId: postInfo.publisherId, myPersonalId: myPersonalId);
                      },
                    ),
                  ],
                ],
